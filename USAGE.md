...
axiom 的已知局限（显式承认，详见 `SKILL.md` 诚实边界段落）：

1. 不能替代实际项目上下文 — 缺少 `.agents/context/` 时系统行为降级
2. 不能保证架构决策正确 — 规范是流程约束，不是正确性保证
3. 不适用于快速原型和 MVP 验证 — 流程过重，建议 Micro-Task
4. 不适用于一次性脚本和数据探索 — 测试覆盖率等要求不现实
5. 版本管理不够严格 — 源模块 v20.0 与蒸馏版 v21.1.0 版本号未同步
6. 性能量化基准有限 — 标准 F 已增加可操作指标，但领域特定阈值需项目自行定义

不适合的场景详见 `README.md` 显式不适用声明。
