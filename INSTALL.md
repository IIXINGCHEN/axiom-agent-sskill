...
**仓库**：`https://github.com/IIXINGCHEN/axiom-agent-skill`
...
git clone https://github.com/IIXINGCHEN/axiom-agent-skill.git <技能目录>/axiom
```
...
git clone https://github.com/IIXINGCHEN/axiom-agent-skill.git ~/.claude/skills/axiom
...
git clone https://github.com/IIXINGCHEN/axiom-agent-skill.git ~/.agents/skills/axiom
...
2. 点击 **"+"** → **导入技能** → 选择 **GitHub**
3. 粘贴仓库 URL：`https://github.com/IIXINGCHEN/axiom-agent-skill`
...
# 工作区级
mkdir -p .kiro/skills
git clone https://github.com/IIXINGCHEN/axiom-agent-skill.git .kiro/skills/axiom
...
REPO_URL="https://github.com/IIXINGCHEN/axiom-agent-skill.git"
SKILL_NAME="axiom"
...
# 或 clone 后本地执行
git clone https://github.com/IIXINGCHEN/axiom-agent-skill.git /tmp/axiom-install
cd /tmp/axiom-install
chmod +x install.sh
./install.sh
```

> **Windows 用户**：在 Git Bash 中可直接执行上述脚本。或使用下方 PowerShell 脚本。
...
```powershell
# install.ps1 — axiom-agent-skill Windows 全平台安装
$ErrorActionPreference = "Stop"

$RepoUrl = "https://github.com/IIXINGCHEN/axiom-agent-skill.git"
$SkillName = "axiom"
...
function Install-To {
    ...
    if (Test-Path $dest) {
        Write-Host "[跳过] $Label — 已存在: $dest"
        return
    }

    git clone $RepoUrl $dest
    Write-Host "[已安装] $Label → $dest"
}

# Claude Code
Install-To -TargetDir "$env:USERPROFILE\.claude\skills" -Label "Claude Code (全局)"

# OpenAI Codex CLI
Install-To -TargetDir "$env:USERPROFILE\.agents\skills" -Label "Codex CLI (全局)"

# Amazon Kiro IDE
Install-To -TargetDir "$env:USERPROFILE\.kiro\skills" -Label "Kiro IDE (全局)"

Write-Host ""
Write-Host "安装完成。在对应工具中提及「axiom」「axiomOS」即可激活。"
Write-Host "更新技能: 在各 skills/axiom/ 目录下执行 git pull"
```
...
# 或 clone 后本地执行
git clone https://github.com/IIXINGCHEN/axiom-agent-skill.git $env:TEMP\axiom-install
cd $env:TEMP\axiom-install
