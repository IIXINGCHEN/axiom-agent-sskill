...
| 安全影响 | 无 | 低（内部 API） | 高（外部暴露/数据变更） |
| 回滚成本 | 低（单文件） | 中（多文件） | 高（数据库/部署） |

**路由规则**:
- 全部"简单" → Micro-Task
- 任意"中等" → 轻量 Spec + HARD-GATE
- 任意"复杂"或 ≥3 个"中等" → SDM 完整流程
...
5. **Self-Diagnostic 开销 vs 效率**：每次回复都生成 YAML 状态报告 vs 简单问答效率。
   - **必须生成**: 涉及代码变更、架构建议、安全评估、Bug 诊断、模式选择
   - **可省略**: 文件路径查询、命令用法、概念解释、配置确认
...
5. **版本管理不够严格** — 源文件标 v20.0.0，蒸馏版 SKILL.md 和 README 已升至 v21.0，但源模块版本号未同步更新，说明维护流程仍有漏洞。
6. **性能量化基准有限** — 标准 F 已增加可操作指标（避免 N+1、O(n²) 注释、超时设置），但领域特定的性能阈值仍需项目自行定义。
...
> 路径约定：`<AXIOM_HOME>` = axiom 命名空间规范目录；`<AXIOM_SKILL>` = 旧版 axiom SKILL 项目；`<NUWA_SKILL>` = [nuwa-skill 方法论项目](https://github.com/alchaincyf/nuwa-skill)；`<CLAUDE_MD>` = 全局 Agent 约束配置。
